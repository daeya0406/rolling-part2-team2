...
function TestAvatar({
  ...
  const avatarClasses = [
    "test-avatar",
    `test-avatar--${size}`,
    isCount ? "test-avatar--count" : "",
    isCount && isListMode ? "test-avatar--count--list" : "",
    onClick ? "test-avatar--selectable" : "",
    isSelected ? "test-avatar--selected" : "",
    className,
  ]
    .filter(Boolean)
    .join(" ");
  ...
  if (isCount) {
    return (
      ...
      >
        <span className="test-avatar__count-text">+{count}</span>
      </div>
    );
  }
  ...
  return (
    <div className={avatarClasses} onClick={onClick ? handleClick : undefined}>
      <img src={src} alt={alt} className="test-avatar__image" />
    </div>
  );
}
